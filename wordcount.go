...
//PariList 实现了sort接口，可以使用sort.Sort对其排序
...
/*
    基于map实现了类型WordCount,并对期实现了Merge(), Report(), SortReport(), UpdateFrep(),
    WordFreqCounter() 方法
*/
...
func (wordcount WordCount) SortReport() {
    ...
    i := 0
    for k, v := range wordcount { //将wordcount map 转换成PairList
        p[i] = Pair{k, v}
        i++
    }
...
func (wordcount WordCount) UpdateFreq(filename string) {
    ...
    if file, err = os.Open(filename); err != nil {
        log.Println("failed to open the file: ", error)
        return  
    }
